$(document).ready(function () {
  ...
  function showFileInput(type) {
    $('#chat-file').data('attachment-type', type);
    $('#chat-file').click();
  ...
  function startVideoRecording() {
    ...
    navigator.mediaDevices.getUserMedia({ video: !0, audio: !0 })
      .then((stream) => {
        mediaRecorder = new MediaRecorder(stream);
        recordedChunks = [];
    ...
    setTimeout(() => {
      if (!map) {
        ...
        L.tileLayer('https://{s}.tile.openstreetmap.org/{z}/{x}/{y}.png')
          .addTo(map);
        marker = L.marker([20.5937, 78.9629], { draggable: !0 }).addTo(map);
  ...
  $('#btnSendLocation').click(() => {
    ...
    if (dataChannel && dataChannel.readyState === 'open') {
      ...
      displayMessage('Me', message, !0, 'location', null, message.messageId, 'sent');
    }
  ...
  $('#saveGeminiKeyBtn').click(() => {
    ...
    showAlert("Gemini API key saved successfully", !1);
  });
  ...
  $('#ai-btn').click(() => {
    ...
    $('#btn-send-text').off('click').on('click', async () => {
      ...
      if (!msg) return;
      displayMessage("You", msg, !0, 'text', null, Date.now().toString(), 'sent');
      $('#chat-message').val('');
      const reply = await getGeminiReply(msg);
      displayMessage("Jarvis", reply, !1, 'text', null, Date.now().toString(), 'delivered');
    });
      ...
      const response = await fetch(
        `https://generativelanguage.googleapis.com/v1beta/models/gemini-2.0-flash:generateContent?key=${apiKey}`,
        {
          ...
          body: JSON.stringify({
            contents: [{
              parts: [{
                text: `Reply like a friendly human:\n${userMessage}`
              }]
            }]
          })
        }
